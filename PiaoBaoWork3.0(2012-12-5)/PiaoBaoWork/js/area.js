...
where[23] = new comefrom("四川", "|成都|绵阳|德阳|自贡|攀枝花|广元|内江|乐山|南充|宜宾|广安|达州|雅安|眉山|甘孜|凉山|泸州|遂宁|资阳|巴中|阿坝");  
...
where[31] = new comefrom("新疆","|乌鲁木齐|石河子|克拉玛依|伊犁|巴音郭勒|昌吉|克孜勒苏柯尔克孜|博尔塔拉|吐鲁番|哈密|喀什|和田|阿克苏|阿泰勒|塔城");  
...
function initxiugai(a, b) {
    with (document.getElementById('province')) {
        ...
        options[selectedIndex].text = a; options[selectedIndex].value = a;
    }
    ...
    for (i = 0; i < where.length; i++) {
        if (where[i].loca == loca2) {
            ...
            document.getElementById('city').options[document.getElementById('city').selectedIndex].text = b;
            document.getElementById('city').options[document.getElementById('city').selectedIndex].value = b;
            break;
        }
    }
...
function clearprovince() {
    with (document.getElementById('province')) {
        options[0].text = where[0].loca;
        options[0].value = where[0].loca;
        var loca2 = options[0].value;
        document.getElementById("hidPro").value = loca2;
    }
    with (document.getElementById('city')) {
        options[0].text = where[0].locacity;
        options[0].value = where[0].locacity;
        var loca2 = options[0].value;
        document.getElementById("hidCity").value = loca2;
    }
}
        ...
        if (where[i].loca == loca2) {
            ...
            for (j = 0; j < loca3.length; j++) {
                with (document.getElementById('city')) {
                    ...
                    if (j == 0) {
                        options[0].text = where[0].locacity;
                        options[j].value = where[0].locacity;
                    }
                    var loca4 = options[selectedIndex].value;
...
function citySelect() {
    var loca2 = document.getElementById("hidPro").value;
    var loca4 ="";
    for (i = 0; i < where.length; i++) {
        if (where[i].loca == loca2) {
            loca3 = (where[i].locacity).split("|");
            for (j = 0; j < loca3.length; j++) {
                with (document.getElementById('city')) {
                    ...
                    var hidCityStr =document.getElementById("hidCity").value;
                    if (hidCityStr != "") {
                        loca4 = hidCityStr;
                    } else {
    ...
    document.getElementById("hidCity").value = loca4;
 }

function init() {
    if (where != null && document.getElementById('province')) {
        with (document.getElementById('province')) {
            options.length = where.length;
            for (k = 0; k < where.length; k++) {
                options[k].text = where[k].loca;
                options[k].value = where[k].loca;
            }
    ...
    var pro = document.getElementById("hidPro").value;
