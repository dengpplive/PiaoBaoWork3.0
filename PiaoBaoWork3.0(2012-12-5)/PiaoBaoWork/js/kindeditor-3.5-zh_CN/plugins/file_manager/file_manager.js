...
KE.event.ready(function () {
    ...
    var insertImage = function (url, title) {
        if (!insertLink(url)) {
            KE.util.insertHtml(id, '<img src="' + url.replace("../attached", "kindeditor-3.5-zh_CN/attached") + '" alt="' + title + '" border="0" />');
        }
    };
    var insertFile = function (url, title) {
        if (!insertLink(url)) {
            KE.util.insertHtml(id, '<a href="' + url.replace("../attached", "kindeditor-3.5-zh_CN/attached") + '" target="_blank">' + title + '</a>');
        }
    };
