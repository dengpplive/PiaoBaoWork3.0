...
public partial class Sys_BaseCabinList : BasePage
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            btnAdd.PostBackUrl = string.Format("BaseCabinEdit.aspx?currentuserid={0}", Request["currentuserid"].ToString());
            Curr = 1;
            Con = " 1=1";
            AspNetPager1.PageSize = 20;
            ViewState["orderBy"] = " AirCode,Rebate desc";
    ...
    private void BaseCabinListDataBind()
    {
        ...
        outParams.Add("1", "out");
        List<Bd_Air_BaseCabin> list = baseDataManage.CallMethod("Bd_Air_BaseCabin", "GetBasePager", outParams, new object[] { TotalCount, AspNetPager1.PageSize, Curr, "*", Con, ViewState["orderBy"].ToString() }) as List<Bd_Air_BaseCabin>;
    ...
    protected void repCabinList_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        try
        {
            ...
            {
                msg = (bool)baseDataManage.CallMethod("Bd_Air_BaseCabin", "DeleteById", null, new Object[] { id }) == true ? "删除成功" : "删除失败";
            }
    ...
    private string SelWhere()
    {
        ...
        {
            StrWhere.Append(" and Cabin = '" + txtCabin.Text.Trim().Replace("'", "") + "'");
        }
        if (txtRebate.Text.Trim() != "")
        {
            StrWhere.Append(" and Rebate = " + txtRebate.Text.Trim().Replace("'", "") + " ");
        }
