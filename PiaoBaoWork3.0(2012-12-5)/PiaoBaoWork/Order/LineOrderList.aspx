<%@ Page Language="C#" AutoEventWireup="true" CodeFile="LineOrderList.aspx.cs" Inherits="Order_LineOrderList" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
            ...
            $J("#show").dialog({
                ...
                buttons: {
                    '确定': function (evt) {
                        ...
                        if(f!=null) {
                            //默认不取消编码
                            var isCancelpnr="0";//$J("#ckpnr").attr("checked")?"0":"1";
        ...
        function UpdateOrder(model) {
            ...
            var val_OpPage="LineOrderList.aspx?currentuserid=<%=this.mUser.id.ToString() %>";
            var param={
        ...
        function showCancel(model) {
            ...
            if($J.trim(model.lockU)!=""&&$J.trim(model.lockU)!=LoginName) {
                ...
                html="<table style='width:100%'><tr><td>确定要取消该订单吗?</td></tr><tr id='hide'><td align='left' class='hide'><label for='ckpnr'><input type='checkbox' id='ckpnr' checked='checked' />保留编码</label></td></tr></table>";
...
<body>
    ...
    <div style="top: 0px;">
        <div class="title">
            <asp:Label ID="lblShow" Text="线下订单查询" runat="server" />
        </div>
                ...
                <table class="Search" cellspacing="3" cellpadding="0" border="0">
                    <tbody>
                        ...
                        <tr id="More">
                            ...
                            <td>
                                <asp:DropDownList ID="ddlStatus" Width="115px" runat="server">
                                    <asp:ListItem Text="全部状态" Value="0" Selected="True"></asp:ListItem>
                                </asp:DropDownList>
                            </td>
                            <th>
                                航空公司：
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                ...
                                <span class="btn btn-ok-s">
                                    <asp:Button ID="btnClear" runat="server" Text="重置数据" OnClientClick="return resetClear();" /></span>
                            </td>
                        </tr>
                    </tbody>
                </table>
                        ...
                        <ItemTemplate>
                            <tr onmouseout="this.bgColor='#ffffff';" onmouseover="this.bgColor='#F5F5F5';">
                                ...
                                <td class="Operation" style="width: 10%;">
                                    <a href='OrderDetail.aspx?id=<%# Eval("id")%>&Url=LineOrderList.aspx&currentuserid=<%=this.mUser.id.ToString() %>'>
                                        订单详情</a><br />
                                    <a href="#" onclick="return showCancel({orderid:'<%#Eval("OrderId") %>',oid:'<%#Eval("id") %>',lockU:'<%#Eval("LockLoginName") %>',pnr:'<%#Eval("PNR") %>',office:'<%#Eval("Office") %>'})"
                                        class='<%# ShowText(5,Eval("OrderStatusCode")) %>'>取消订单</a>
                                </td>
                            </tr>
                        </ItemTemplate>
                    </asp:Repeater>
